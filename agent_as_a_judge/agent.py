...
class JudgeAgent:
    ...
    def check_requirement(self, criteria: str, workflow: list, user_query: str):
        ...
        def format_reason(reason_list):
            """
            Format the reason from a list of strings into a Markdown-friendly format.
            """
            if isinstance(reason_list, list) and reason_list:
                reason = reason_list[0]  # Assuming the first element contains the main reason
            else:
                return "No reason provided."

            # Add Markdown headings and clean up formatting
            formatted_reason = reason.replace("\n\n", "\n\n")  # Preserve newlines for readability
            formatted_reason = f"{formatted_reason}"
        ...
        self.display_judgment(
            ...
            reason=format_reason(check_llm_stats["reason"]),
            logger=logging,
        )
